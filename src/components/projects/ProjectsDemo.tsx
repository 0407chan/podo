...
export function ProjectsDemo() {
    ...
    return (
        <div>
            ...
            {isError && <div style={{ color: 'crimson' }}>{(error as any)?.message ?? 'Failed to load'}</div>}

            <ul style={{ display: 'grid', gap: 8, padding: 0, listStyle: 'none' }}>
                {(data ?? []).map((p) => (
                    <li key={p.id} style={{ border: '1px solid #ddd', borderRadius: 8, padding: 12 }}>
                        <div style={{ fontWeight: 600 }}>{p.name}</div>
                        {p.description && <div style={{ color: '#666', marginTop: 4 }}>{p.description}</div>}
                        <div style={{ marginTop: 6, fontSize: 12, color: '#999' }}>Created: {new Date(p.created_at).toLocaleString()}</div>
                    </li>
                ))}
            </ul>
        </div>
